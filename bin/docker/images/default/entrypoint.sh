...
if [ "$NEED_SETUP_CONFIG" = "1" ]; then
    ...
    n98-magerun2.phar -q --skip-root-check --root-dir="$MAGENTO_ROOT" config:store:set --encrypt hipay/hipay_credentials_applepay/secret_passphrase_test $HIPAY_TOKENJS_PUBLICKEY_TEST
